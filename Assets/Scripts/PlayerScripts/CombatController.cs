    ...
    private void LockOnSystem()
    {
        ...
        if (!lockedOn) //FIJADO AUTOMÁTICO - SOLO EN CASO DE QUE EL JUGADOR NO HAYA FIJADO UN ENEMIGO.
        {
            ...
            if (lockedEnemy != null)
            {
                ...
                crosshair.transform.position = Camera.main.WorldToScreenPoint(new Vector3 (lockedEnemy.transform.position.x, lockedEnemy.transform.position.y + 1.5f, lockedEnemy.transform.position.z));
